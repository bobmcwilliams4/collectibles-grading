"""
Together AI Vision Grader - Llama 4 Vision Models
https://api.together.ai - OpenAI-compatible API

Updated December 2025: Using Llama 4 Scout/Maverick for best vision quality
"""
...
# Together AI Vision Models (December 2025) - From I:\DOCUMENTATION\USER_MANUALS\TOGETHER.AI
# Per together_ai_full_manual.md: Llama 4 Scout, Llama 4 Maverick, Qwen2.5-VL
TOGETHER_VISION_MODELS = [
    "meta-llama/Llama-4-Scout",              # Llama 4 Scout - vision capable
    "meta-llama/Llama-4-Maverick",           # Llama 4 Maverick - vision capable
    "Qwen/Qwen2.5-VL-72B-Instruct",          # Qwen2.5 Vision 72B
]
...
async def grade_with_together(
    image_input: str,
    model: str = PRIMARY_MODEL,
    metadata: Optional[Dict] = None
) -> Dict[str, Any]:
    """
    Grade a comic book image using Together AI's vision models
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    logger.error(f"Together API error: {error_text}")

                    # Try fallback model
                    if model == PRIMARY_MODEL and len(TOGETHER_VISION_MODELS) > 1:
                        logger.info("Trying fallback Together model...")
                        return await grade_with_together(
                            image_input,
                            model=TOGETHER_VISION_MODELS[1],
                            metadata=metadata
                        )

                    return {
                        'grade': 0,
                        'error': f"Together API error: {resp.status}",
                        'provider': 'Together'
                    }
    except Exception as e:
        logger.error(f"Together grading error: {e}")
        return {
            'grade': 0,
            'error': str(e),
            'provider': 'Together'
